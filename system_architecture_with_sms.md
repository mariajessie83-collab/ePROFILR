# Gsystem Complete Architecture with All Flows
...
## Detailed Complaint Flow with Arrows
...
1. VICTIM (Student/Teacher) COMPLAINT PROCESS:
   ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
   │ Victim  │───▶│ Sign Up │───▶│ Login   │───▶│ Fill    │
   │ Reports │    │ Process │    │ Process │    │ Form    │
   └─────────┘    └─────────┘    └─────────┘    └─────────┘
        │              │              │              │
        ▼              ▼              ▼              ▼
   ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
   │ Picture │───▶│ File    │───▶│ Submit  │───▶│ POD     │
   │ Upload  │    │ Upload  │    │ Report  │    │ Review  │
   └─────────┘    └─────────┘    └─────────┘    └─────────┘
        │              │              │              │
        ▼              ▼              ▼              ▼
   ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
   │ Store   │───▶│ Store   │───▶│ Store   │───▶│ Send    │
   │ Picture │    │ File    │    │ Report  │    │ SMS     │
   └─────────┘    └─────────┘    └─────────┘    └─────────┘
        │              │              │              │
        ▼              ▼              ▼              ▼
   ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
   │ MySQL   │───▶│ MySQL   │───▶│ MySQL   │───▶│ Parents │
   │ Database│    │ Database│    │ Database│    │ Receive │
   └─────────┘    └─────────┘    └─────────┘    └─────────┘

2. RESPONDENT (Accused) NOTIFICATION PROCESS:
   ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
   │ POD     │───▶│ Check   │───▶│ Get     │───▶│ Send    │
   │ Review  │    │ Database│    │ Contact │    │ SMS     │
   └─────────┘    └─────────┘    └─────────┘    └─────────┘
        │              │              │              │
        ▼              ▼              ▼              ▼
   ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
   │ Identify│───▶│ Get     │───▶│ Format  │───▶│ Respondent│
   │ Respondent│   │ Parent │    │ Message │    │ Parents  │
   └─────────┘    └─────────┘    └─────────┘    └─────────┘
        │              │              │              │
        ▼              ▼              ▼              ▼
   ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
...
### Detailed Method Flow with Arrows:
...
STEP 1: VICTIM COMPLAINT INITIATION
┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
│ Victim  │───▶│ Access  │───▶│ Fill    │───▶│ Upload  │
│ Login   │    │ Form    │    │ Details │    │ Picture │
└─────────┘    └─────────┘    └─────────┘    └─────────┘
     │              │              │              │
     ▼              ▼              ▼              ▼
┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
│ Validate│───▶│ Validate│───▶│ Validate│───▶│ Store   │
│ User    │    │ Form    │    │ Data    │    │ File    │
└─────────┘    └─────────┘    └─────────┘    └─────────┘

STEP 2: POD PROCESSING
┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
│ Receive │───▶│ Review  │───▶│ Identify│───▶│ Process │
│ Report  │    │ Case    │    │ Respondent│   │ Case    │
└─────────┘    └─────────┘    └─────────┘    └─────────┘
     │              │              │              │
     ▼              ▼              ▼              ▼
┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
│ Store   │───▶│ Update  │───▶│ Get     │───▶│ Prepare │
│ Case    │    │ Status  │    │ Parent  │    │ SMS     │
└─────────┘    └─────────┘    └─────────┘    └─────────┘

STEP 3: SMS NOTIFICATION TO PARENTS
┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
│ Format  │───▶│ Send    │───▶│ Track   │───▶│ Parents │
│ Message │    │ SMS     │    │ Delivery│    │ Receive │
└─────────┘    └─────────┘    └─────────┘    └─────────┘
     │              │              │              │
     ▼              ▼              ▼              ▼
┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐
│ Log      │───▶│ Store   │───▶│ Update  │───▶│ Respond │
│ SMS      │    │ Status  │    │ Case    │    │ to SMS  │
└─────────┘    └─────────┘    └─────────┘    └─────────┘
```
...
**VICTIM SIDE:**
- Sign Up → Login → Fill Form → Upload Picture → Submit Report
- All data stored in MySQL Database
- POD receives notification

**RESPONDENT SIDE:**
- POD identifies respondent from complaint
- Gets parent contact information
- Sends SMS to respondent's parents
- Parents receive SMS notification
...
- **Automatic SMS**: POD sends SMS to parents without manual editing
- **Parent Notification**: Parents receive SMS about their child's involvement
- **Case Tracking**: All SMS communications logged in database
- **Status Updates**: Real-time tracking of SMS delivery
- **Response Handling**: Parents can respond to SMS notifications

This detailed flow shows the complete process from victim complaint to parent notification with all the arrows and steps clearly defined.
